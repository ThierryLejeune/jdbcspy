...
public class ConnectionFactory {
        ...
        if (ClientProperties.getBoolean(ClientProperties.DB_DUMP_AFTER_SHUTDOWN)) {
            synchronized (dumpAfterShutdownThread) {
                if (!dumpAfterShutdownThread) {
                    ...
                    Thread oThread = new Thread(() -> System.out.println(dumpStatistics()));

                    oThread.setDaemon(true);
                    Runtime.getRuntime().addShutdownHook(oThread);
                }
            }
        }
            ...
            synchronized (dumpIntervalThread) {
                if (!dumpIntervalThread) {
                    ...
                    Thread t = new Thread(() -> {
                        try {
                            while (true) {
                                Thread.sleep(1000 * ClientProperties.getInt(ClientProperties.DB_DUMP_INTERVAL));
    ...
    public final Connection getProxyConnection(Connection conn) {
        ...
        ConnectionInvocationHandler connHandler = new ConnectionInvocationHandler(conn);
     ...
     * Get the connection.
     *
     * @param inoConnexion the original connection
     * @return a proxy connection
     */
    public final XAConnection getProxyXAConnection(XAConnection inoConnexion) {
        if (!mEnableProxy) {
            // get standard connection
            return inoConnexion;
        }
        XAConnectionInvocationHandler connHandler = new XAConnectionInvocationHandler(inoConnexion, this);
     ...
     * @return a proxy connection
     */
    public final XAResource getProxyXAResource(XAResource xaResource, XAConnectionInvocationHandler handler) {
        if (!mEnableProxy) {
            // get standard connection
            return xaResource;
        }
        XAResourceInvocationHandler xaHandler = new XAResourceInvocationHandler(xaResource, handler);
